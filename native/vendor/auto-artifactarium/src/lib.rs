...
impl GameSniffer {
    ...
    /// Restores the XOR key for an already-running game session.
    pub fn set_session_key(mut self, session_key: Vec<u8>) -> Self {
        self.key = Some(Key::Session(session_key));
        self
    }

    /// Returns the discovered session key, but never a dispatch key.
    pub fn session_key(&self) -> Option<&[u8]> {
        match &self.key {
            Some(Key::Session(key)) => Some(key),
            _ => None,
        }
    }
